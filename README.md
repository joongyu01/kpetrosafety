...
Supabase 무료 프로젝트는 **7일간 활동이 적으면 일시정지**됩니다
([공식 문서](https://supabase.com/docs/guides/platform/free-project-pausing)).
정지 자체는 되돌릴 수 있지만(정지 후 1년 안에 복구), 방치하면 결국 잃습니다.
실제로 첫 프로젝트가 그렇게 사라져 신고 데이터와 비밀번호를 모두 날렸습니다.

그래서 GitHub Actions 가 **하루 두 번, 매회 3건씩** DB 를 건드립니다.

```
06:17 / 18:17 KST ─> POST /rest/v1/rpc/sr_ping  × 3
...
> **왜 하루 한 번이 아닌가요?**
> 공식 문서가 기준을 "하루에 몇 건(a few requests each day)"으로 적고 있습니다.
> 한 번만 보내면 그 한 번이 걸러졌을 때 그날이 통째로 빕니다. 하루 6건이면
> 예약 작업이 한 번 밀려도 나머지 한 번이 그날을 메웁니다.

| 항목 | 값 |
|---|---|
| 워크플로 | [`.github/workflows/keepalive.yml`](.github/workflows/keepalive.yml) |
| 주기 | 하루 2회 × 3건 = **6건/일** (정지 기준 7일보다 훨씬 촘촘) |
| 실패 시 | 건별 20초 간격 3회 재시도 → 3건 모두 실패해야 작업 실패 → **GitHub 이 소유자에게 메일** |
| 일부 실패 | 1~2건만 성공하면 경고만 남김 (DB 는 살아 있으므로) |
| 수동 실행 | Actions 탭 → *Supabase 휴면 방지* → **Run workflow** |
| 접속 정보 | `config.js` 를 그대로 읽음. 저장소 시크릿 `SUPABASE_URL` / `SUPABASE_ANON_KEY` 를 넣으면 그쪽이 우선 |

`sr_ping` 은 다른 함수와 달리 **접근코드를 요구하지 않습니다.** 워크플로 파일이 공개
저장소에 있어 코드를 적어 둘 수 없기 때문입니다. 대신 하는 일이 심박 한 줄 쓰기와
청소뿐이라 아무나 불러도 새어 나가는 정보도, 늘어나는 데이터도 없습니다.
...
> 저장소 Actions 토큰이 읽기 전용이면 이 단계는 실패하지만, 심박과는 무관합니다
> (Settings → Actions → General → Workflow permissions 를 *Read and write* 로).

### 그래도 남는 위험

예약 작업은 GitHub 사정으로 밀리거나 걸러질 수 있고, 저장소를 보관(archive)하면 멈춥니다.
**한 달에 한 번쯤 Actions 탭에 초록불이 이어지는지 봐 주세요.** 정지가 임박하면 Supabase 가
일주일 전에 경고 메일을 보내므로, 그 메일이 오면 심박이 멎은 것입니다.

그리고 휴면 방지는 **백업이 아닙니다.** 무료 플랜에는 백업 복구가 없어서 계정 문제나
실수로 지워지면 그대로 끝입니다 — 분기마다 현황판에서 CSV 를 한 번 내려받아 두세요.
